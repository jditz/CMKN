...
# define alphabets
ALPHABETS = {
    "DNA": ("ACGT", "\x01\x02\x03\x04"),
    "DNA_FULL": ("ACGTN", "\x01\x02\x03\x04" + "\x00"),
    "DNA_GAP": ("ACGTN-", "\x01\x02\x03\x04" + "\x00" * 2),
    "DNA_AMBI": ("N", "\x00"),
    "PROTEIN": (
...
def pad_sequences(
    sequences, pre_padding=0, maxlen=None, padding="pre", alphabet="DNA_AMBI"
):
...
class CMKNDataset(data.Dataset):
    ...
    The CMKNDataset object can handle dataset given as fasta files, directly. To succesfully read ENCODE datasets, the
    ...
    def __init__(
        ...
        if self.ext == "fasta":
            ...
            if clean_set is not None:
                self.data = [
                    i[1] for i in aux if i[0].split("|")[clean_set[0]] != clean_set[1]
                ]
                self.labels = [
                    i[0] for i in aux if i[0].split("|")[clean_set[0]] != clean_set[1]
                ]
            else:
            ...
            if len(self.data) != len(self.labels):
                raise ValueError(
                    "data and labels have to be of the same length\n"
                    + "    Found instead: len(data)={}, len(labels)={}".format(
    ...
    def _encode_onehot(self, idx):
        ...
        try:
            ...
            data_tensor = torch.zeros(
                len(idx), len(ALPHABETS[self.alphabet][0]), self.seq_len
            )
            ...
            for i, sample in enumerate(idx):
                ...
                data_tensor[i, :, :] = [
                    [0 if char != letter else 1 for char in self.data[sample]]
                    for letter in ALPHABETS[self.alphabet][0]
                ]
            ...
            data_tensor = torch.tensor(
                [
                    [0 if char != letter else 1 for char in self.data[idx]]
                    for letter in ALPHABETS[self.alphabet][0]
                ],
                dtype=torch.float,
            )
    ...
    def update_dataset(self, tfid=None, split="train"):
        ...
        if isinstance(tfid, str):
            # load sequences and labels
            self.data, self.labels = self.data_loader.get_dataset(
                tfid, val_split=0, split=split
            )
        elif hasattr(tfid, "__iter__"):
            self.data = self.labels = []
            # iterate over all provided ids and combine sequences and labels
            for s in tfid:
                aux_data, aux_labels = self.data_loader.get_dataset(
                    s, val_split=0, split=split
                )
                self.data = [*self.data, *aux_data]
                self.labels = [*self.labels, *aux_labels]
        else:
            raise ValueError(
                "Expected String or [String] for tfid; received {}".format(tfid)
            )
...
class EncodeLoader(object):
    ...
    def __init__(
        self, datadir, ext=".seq.gz", maxlen=None, pre_padding=0, alphabet="DNA"
    ):
        self.alphabet, self.code = ALPHABETS[alphabet]
        alpha_ambi, code_ambi = ALPHABETS[alphabet + "_AMBI"]
        self.translator = maketrans(alpha_ambi + self.alphabet, code_ambi + self.code)
    ...
    def pad_seq(self, seq):
        seq = pad_sequences(
            seq, pre_padding=self.pre_padding, maxlen=self.maxlen, padding="post"
        )
    ...
    def get_ids(self, ids=None):
        targetnames = sorted(
            [
                filename.replace("_AC" + self.ext, "")
                for filename in os.listdir(self.datadir)
                if filename.endswith("_AC" + self.ext)
            ]
        )
        if ids is not None and ids != []:
            targetnames = [targetnames[tfid] for tfid in ids if tfid < len(targetnames)]
        return targetnames
    ...
    def get_dataset(
        self, dataid, split="train", val_split=0.25, top=False, generate_neg=True
    ):
        ...
        X = self.pad_seq(X)
        if top:
            X, _, y, _ = train_test_split(X, y, stratify=y, train_size=500)
