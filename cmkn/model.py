...
from .utils import ClassBalanceLoss, category_from_output
...
class CMKN(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cmkn_layer = CMKNLayer(
            in_channels,
            out_channels_list[0],
            filter_sizes[0],
            padding=paddings[0],
            subsampling=strides[0],
        ...
        if isinstance(scaler, int):
            if pool_global is None:
                ...
                #   -> this is the number of output channels of the last CMKN layer multiplied by the sequence length
                ...
                self.fc = nn.Linear(
                    self.out_features, self.num_classes * 100, bias=fit_bias
                )
                self.classifier = nn.Linear(
                    self.num_classes * 100, self.num_classes, bias=fit_bias
                )
            else:
                # set the number of output features
                ...
                self.classifier = nn.Linear(
                    self.out_features, self.num_classes, bias=fit_bias
                )
        else:
    ...
    def predict(
        self, data_loader, only_representation=False, proba=False, use_cuda=False
    ):
        ...
        for i, (oli_in, target, *_) in enumerate(data_loader):
            ...
            if i == 0:
                ...
                target_output = torch.Tensor(n_samples, target.shape[-1]).type_as(
                    target
                )
    ...
    def initialize(
        ...
        try:
            n_oligomers_per_batch = max(
                (n_sampling_motifs + len(data_loader) - 1) // len(data_loader), 500
            )
        except:
            n_oligomers_per_batch = 500
        print(
            "        {} oligomers per batch will be sampled".format(
                n_oligomers_per_batch
            )
        )
        ...
        if distance == "euclidean":
            ...
            oligomers = self.cmkn_layer.weight.new_zeros(
                n_sampling_motifs, self.cmkn_layer.patch_dim + 1
            )
            ...
            # initialize tensor that stores the sampled oligomers and make sure it is on the same device as the model
            oligomers = self.cmkn_layer.weight.new_zeros(
                n_sampling_motifs, self.cmkn_layer.patch_dim
            )
        ...
        for data, _ in data_loader:
            ...
            with torch.no_grad():
                data_oliogmers = self.cmkn_layer.sample_oligomers(
                    data, n_oligomers_per_batch, include_pos
                )
        ...
        self.cmkn_layer.initialize_weights(
            distance, oligomers, seq_len, init=init, max_iters=max_iters
        )
    ...
    def sup_train(
        ...
        """Perform supervised training of the CMKN model
                ...
                for oli, target, *_ in data_loader[phase]:
                    ...
                    if phase == "val":
                        with torch.no_grad():
                            ...
                            if self.num_classes > 2 or isinstance(
                    ...
                    running_corrects += torch.sum(
                        torch.sum(pred == target.data, 1)
                        == target.new_ones(pred.shape[0]) * self.num_classes
                    ).item()
...
class PhyloCMKN(nn.Module):
    """Phylogeny Convolutional Motif Kernel Network

    This class implements a CMKN model that utilizes phylogeny information to solve a prediction task.
    The model will have a simple structure and does not offer the flexibility of the CMKN class. However,
    it can be used as a starting point to develop custom PhyloCMKN models.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            in_channels (:obj:`int`): Dimensionality of the alphabet that generates the input sequences.
            num_anchors (:obj:`int`): Number of output channels (i.e., anchor points) of the CMKNLayer.
            kmer_size (:obj:`int`): Length of motifs used for the kernel evaluation.
            stride (:obj:`int`): Stride factor of the Conv1d layer. For the CMKNLayer this
                argument determines the subsampling property.
            padding (:obj:`str`): Padding factor for the CMKNLayer.
            seq_len (:obj:`int`): Length of the input sequences. This model utilizes a linear fully
                connected layer for classification and the Integer determines the input
                dimensionality of that layer.
            mask_seq_dict (:obj:`dict`): This dictionary contains as key the name of the sequences used to create
                phylogeny masks and as items the corresponding sequences as one-hot-encoded tensors.
            kernel_funcs (:obj:`str`): Specifies the kernel function used in the CMKNLayer. Defaults to None.
            kernel_args_list (:obj:`tuple` of :obj:`int`): Tuple of arguments for the used kernel. Defaults to None.
            kernel_args_trainable (:obj:`bool`): Indicates for the CMKNLayer if the kernel parameters used in this
                layer are trainable.
        ...
        # initialize parent class
        super(PhyloCMKN, self).__init__()

        # store parameters used for the initialization
        self.num_classes = num_classes
        self.alphabet_len = in_channels
        ...
        # store the dictionary containing all sequences that are used to
        # create phylogeny masks
        self.phylo_masks = mask_seq_dict
        ...
        # set the number of output features
        #   -> this is the number of anchor points multiplied by the sequence length
        self.out_features = num_anchors * seq_len
        self.global_pool = None

        # initialize two FC layer for stability reason
    ...
    def initialize_(
        ...
        # initialize the Motif Kernel layer
        print("    Initializing Motif Kernel Layer")
        n_kmers = 0

        # determine the number of kmers sampled from each batch
        #   -> if this number cannot be calculated or is below 1000, sample 1000 kmers from each batch until
        #      n_sampling_olis are sampled
        try:
            n_kmers_per_batch = max(
                (n_sampling_motifs + len(data_loader) - 1) // len(data_loader), 500
            )
        except:
            n_kmers_per_batch = 500
        print(f"        {n_kmers_per_batch} motifs per batch will be sampled")

        # depending on the chosen distance measure for the k-Means algorithm, motifs will incorporate positional
        # information
        if distance == "euclidean":
            # initialize tensor that stores the sampled motifs and make sure it is on the same device as the model
            motifs = self.cmkn_layer.weight.new_zeros(
                n_sampling_motifs, self.cmkn_layer.patch_dim + 1
            )

            # make sure that motifs contain positional information
            include_pos = True
        else:
            # initialize tensor that stores the sampled motifs and make sure it is on the same device as the model
            motifs = self.cmkn_layer.weight.new_zeros(
            ...
            # make sure that motifs will not contain positional information
            include_pos = False
        ...
        for data, *_ in data_loader:
            ...
            # sample the specified number of oligomers using the current batch of data
            with torch.no_grad():
                data_motifs = self.cmkn_layer.sample_oligomers(
                    data, n_kmers_per_batch, include_pos
                )

            # only use a subset of the sampled oligomers in this batch, if this batch would exceed the maximum number
            # of sampled oligomers
            size = data_motifs.size(0)
            if n_kmers + size > n_sampling_motifs:
                size = n_sampling_motifs - n_kmers
                data_motifs = data_motifs[:size]

            # update the oligomer tensor with the currently sampled oligomers and the amount of already sampled
            # oligomers
            motifs[n_kmers : n_kmers + size] = data_motifs
            n_kmers += size

        # initialize the motif and position anchor points of the Motif Kernel layer
        print(f"        total number of sampled motifs: {n_kmers}")
        self.cmkn_layer.initialize_weights(
    ...
    def forward(self, x_in, keys, proba=False):
        ...
        if proba and (self.num_classes == 1):
            return x_out.sigmoid()
        elif proba:
            return F.softmax(x_out, dim=1)
        else:
    ...
    def sup_train(
        ...
        for epoch in range(epochs):
            ...
            for phase in phases:
                ...
                # iterate over the data that will be used in the current phase
                for in_seq, phylo_key, target, *_ in data_loader[phase]:
                    size = in_seq.size(0)
                    target = target.float()
    ...
    def predict(self, data_loader, proba=False, use_cuda=False):
        ...
        # iterate over all samples
        batch_start = 0
        for i, (seq_in, phylo_key, target, *_) in enumerate(data_loader):
            ...
            # do not keep track of the gradients during the forward propagation
            with torch.no_grad():
                batch_out = self(seq_in, phylo_key, proba).data.cpu()
