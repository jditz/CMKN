...
from cmkn import CMKN, CMKNDataset, ClassBalanceLoss, compute_metrics
...
class CustomHandler(CMKNDataset):
    def __init__(self, filepath, kmer_size=3, drug='SQV', nb_classes=2, clean_set=True, encode='onehot',
                 alphabet='PROTEIN_FULL', experiment='hiv'):
        ...
        super(CustomHandler, self).__init__(filepath, kmer_size=kmer_size, alphabet=alphabet,
        ...
        if nb_classes == 2:
            ...
            if experiment == 'hiv':
                ...
                raise ValueError('Unsupported number of classes for splice site prediction '
...
def load_args():
    ...
    parser.add_argument('--type', dest='type', default='HIV', type=str,
                        choices=['HIV', 'SPLICE_NN269', 'SPLICE_DGSPLICER'],
    ...
    parser.add_argument("--splice-type", dest='splice_type', default='acceptor', type=str,
                        choices=['acceptor', 'donor'],
                        help="type of the used splice site (this option is only used if the experiment is set to " +
                             "'SPLICE')")
    parser.add_argument("--loss-beta", dest='loss_beta', default=.99, type=float,
                        help="beta parameter of the ClassBalanceLoss class.")
    parser.add_argument("--loss-gamma", dest='loss_gamma', default=1.0, type=float,
    ...
    if args.type == 'SPLICE_NN269':
        args.filepath = args.filepath.rstrip(os.path.sep) + os.path.sep + 'NN269_{}_'.format(args.splice_type) + \
                        '{}.fasta'
    elif args.type == 'SPLICE_DGSPLICER':
        args.filepath = args.filepath.rstrip(os.path.sep) + os.path.sep + 'DGSplicer_{}_'.format(args.splice_type) + \
                        '{}.fasta'
...
def train_hiv(args):
    ...
    for fold_nb, fold in enumerate(folds):
        ...
        if args.num_classes == 2:
            criterion = ClassBalanceLoss(args.class_count, args.num_classes, 'sigmoid', args.loss_beta, args.loss_gamma)
        else:
            criterion = ClassBalanceLoss(args.class_count, args.num_classes, 'cross_entropy', args.loss_beta,
                                         args.loss_gamma)
        ...
        acc, loss = model.sup_train(loader_train, criterion, optimizer, lr_scheduler, epochs=args.nb_epochs,
                                    early_stop=False, use_cuda=args.use_cuda, kmeans_init='k-means++',
                                    distance='euclidean')
        ...
        torch.save({'args': args, 'state_dict': model.state_dict(), 'acc': acc, 'loss': loss,
                   ...
                   args.outdir + "/CMKN_results_epochs" + str(args.nb_epochs) + "_fold" + str(fold_nb) + ".pkl")
        ...
        try:
            ...
            # plt.show()
            plt.savefig(args.outdir + "/acc_loss_{}.png".format(fold_nb))
            ...
            # plt.show()
            plt.savefig(args.outdir + "/anchor_positions_{}.png".format(fold_nb))
...
def train_splice(args):
    ...
    # load data
    data_all = CustomHandler(args.filepath.format('train'), kmer_size=args.kmer_size, clean_set=False, encode='onehot',
                             alphabet='DNA_FULL', experiment='splice')

    # get labels of each entry for stratified shuffling and distribution of classes for class balance loss
    args.class_count, args.expected_loss, label_vec = count_classes_splice(args.filepath, mode='train')
    ...
    for fold_nb, split_idx in enumerate(skf.split(indices, label_vec)):
        ...
        if args.type == 'SPLICE_DGSPLICER':
            ...
            rus = RandomUnderSampler(sampling_strategy=0.25, random_state=args.seed, replacement=False)
