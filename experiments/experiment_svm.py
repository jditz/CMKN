...
try:
    sys.path.append("../OligoSVM/")
    from OligoEncoding.oligo_encoding import oligoEncoding
    from OligoKernel.oligo_kernel import oligoKernel
except ModuleNotFoundError:
    print('WARNING: The non-public oligo kernel implementatino used in this experiment was not found. Please make sure '
...
def load_args():
    ...
    parser.add_argument("--kernel", dest="kernel", default="poly", type=str, choices=["poly", "oligo"],
    ...
    parser.add_argument("--k_params", dest="k_params", default=[1], type=list,
    ...
    parser.add_argument("--sigma_params", dest="sigma_params", default=[1, 2, 4, 8, 16], type=list,
    ...
    if args.kernel == "oligo" and not os.path.exists(os.path.join(os.getcwd(), '../OligoSVM')):
        raise NotImplementedError('Please provide an implementation of the oligo kernel and replace this Error with'
    ...
    if args.outdir != "":
        args.save_logs = True
        args.outdir = args.outdir + "/SVM_experiment/{}/{}/{}_{}/".format(args.kernel, args.type, args.hiv_type,
                                                                          args.hiv_name)
...
def load_data_hiv(type, drug_number, encode_type='ordinal'):
    ...
    if encode_type == 'oligo':
        seq = [str(i.seq).replace("~", "X") for i in tmp if i.id.split('|')[drug_number] != 'NA']
    else:
...
def experiment(args):
    ...
    if args.kernel == "poly":
        ...
        if len(args.sigma_params) > len(args.k_params):
            aux_params = [list(zip(args.k_params, each_permutation)) for each_permutation in
                          itertools.permutations(args.sigma_params, len(args.k_params))]
        else:
            aux_params = [list(zip(each_permutation, args.sigma_params)) for each_permutation in
                          itertools.permutations(args.k_params, len(args.sigma_params))]
    else:
    ...
    for reg in args.reg_params:
        for param in aux_params:
            ...
            for fold_nb, fold in enumerate(folds):
                ...
                if args.kernel == "poly":
                    clf = svm.SVC(C=reg, kernel=args.kernel, degree=param, probability=True)
                    in_train = X[fold[0]]
                    in_test = X[fold[1]]
                elif args.kernel == "oligo":
                    ...
                    positions, values = oligoEncoding(k_mer_length=param[0][0],
                                                      alphabet=PROTEIN[:-1]).getEncoding(list(sequences))
                    kernel = oligoKernel(sigma=param[0][1], max_distance=True).symmetric(positions, values)
...
def evaluation(args):
    ...
    for reg in args.reg_params:
        for param in aux_params:
            try:
                with open(args.outdir + '/validation_results_{}_{}.pkl'.format(param, reg), 'rb') as in_file:
            ...
            with pd.option_context('display.max_rows', None, 'display.max_columns', None, 'display.width', None):
                print(aux_print.format(param[0], reg))
                print(df_res)
                print('\n--------------------------------------------------------------------------------\n\n')
