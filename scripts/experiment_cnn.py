...
from cmkn import ClassBalanceLoss, compute_metrics
...
def load_args():
    ...
    parser.add_argument(
        ...
        default=50,
        type=int,
        help="Number of kernels in the convolutional layer.",
    )
    ...
    if args.outdir != "":
        ...
        if args.type == "HIV":
            args.nettype = args.hiv_type
            args.outdir = args.outdir + "/CNN_experiment/{}/{}_{}/".format(
                args.type, args.hiv_type, args.hiv_name
            )
        else:
            args.nettype = args.type
            args.outdir = args.outdir + "/CNN_experiment/{}/".format(args.type)
...
def encoding(in_str, alphabet, type="ordinal"):
    if type == "ordinal":
        ...
        vector = [
            [0 if char != letter else 1 for letter in in_str] for char in alphabet
        ]
    else:
...
class Net(nn.Module):
    def __init__(self, nettype, nb_kernels):
        # call __init__ of parent class
        super(Net, self).__init__()

        if nettype == "synthetic":
            out_len = 100
            k_size = 5
        elif nettype == "PI":
            out_len = 99
        ...
        self.linear_layers = nn.Sequential(
            nn.Linear(in_features=nb_kernels * out_len, out_features=200),
            nn.Linear(in_features=200, out_features=2),
        )

    def forward(self, x_in, proba=False):
        x_out = self.embedding(x_in)
        x_out = self.cnn_layer(torch.transpose(x_out, 1, 2))
...
class CustomDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        seq = encoding(self.data[item].seq, self.alphabet, self.encode_type)
        ...
        if self.type == "HIV":
            ...
            label[self.class_to_idx[id_str.split("_")[-1]]] = 1.0
...
def experiment(args):
    ...
    if args.type == "HIV":
        ...
        data_all = CustomDataset(
            "../data/hivdb/{}_DataSet.fasta".format(args.hiv_type),
            extension="fasta",
            exp_params=[args.type, args.hiv_number],
        )

        # get class count for ClassBalanceLoss
        args.class_count, args.expected_loss, _ = count_classes(
            "../data/hivdb/{}_DataSet.fasta".format(args.hiv_type),
            False,
            args.hiv_number,
        )
        ...
        # create dataset handle
        data_all = CustomDataset(
            args.synthetic_data, extension="fasta", exp_params=[args.type, -1],
        )

        # get class count for ClassBalanceLoss
        args.class_count, args.expected_loss = (
            [int(len(data_all) / 2), int(len(data_all) / 2)],
            0.5,
        )
    ...
    for fold in folds:
        ...
        model = Net(args.nettype, args.cnn_kernels)
        ...
        loader_val = data.DataLoader(
            data_val, batch_size=args.batch_size, shuffle=False
        )
        ...
        for i, (inputs, labels, *_) in enumerate(loader_val):
            ...
            if i == 0:
                ...
                target_output = torch.Tensor(n_samples, labels.shape[-1]).type_as(
                    labels
                )
    ...
    filename = "validation_results_{}_{}_{}.pkl".format(
        args.cnn_kernels, args.epochs, args.batch_size
    )
...
def evaluation(args):
    ...
    with pd.option_context(
        ...
        print(
            "Results for CNN trained for {} epochs with batch size {}\n".format(
